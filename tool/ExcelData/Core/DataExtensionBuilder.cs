...
public sealed class DataExtensionBuilder : GeneratorBase<DataHelperConfiguration, StatusEvents>
{
    ...
    public override async Task ExecuteAsync()
    {
        ...
        foreach (Flavors flavor in Options.Flavors)
        {
            ...
            await PopulateConsolidatedData(filePath, xssWorkbook, worksheetCount).ConfigureAwait(false);
            ...
            for (int index = 0; index < worksheetCount; index++)
            {
                ...
                await File.AppendAllTextAsync(filePath, await RenderTemplate("PopulateTableDataTemplate", new
                {
                    table = td,
                    dr = dataRows,
            ...
            await File.AppendAllTextAsync(filePath, "}").ConfigureAwait(false);
        }

        await File.AppendAllTextAsync(filePath, "}").ConfigureAwait(false);
    }

    private async Task PopulateConsolidatedData(string filePath, IWorkbook xssWorkbook, int worksheetCount)
    {
        ...
        await File.AppendAllTextAsync(filePath, await RenderTemplate("PopulateConsolidatedDataTemplate", tables
